// ../.wrangler/tmp/bundle-aBXFsR/checked-fetch.js
...
// ../src/lib/har_sanitize.tsx
var defaultURLSchemas = [
  {
    regex: /https:\/\/vmedia.command.verkada.com\/library\/enckey\//,
    redactRequest: false,
    redactResponse: true
  }
];
...
function removeContentForMimeTypes(input, scrubList) {
  ...
  for (const entry of entries) {
    ...
    if (response.content?.mimeType == "application/json") {
      response.content.text = response.content.text.replaceAll(/"lanServerAuth":"[a-z0-9-|]+:/g, '"lanServerAuth":"[lanServerAuth redacted]:');
      response.content.text = response.content.text.replaceAll(/"credential":"[a-zA-Z0-9!@#$^%&*()\-_=+]+"/g, '"credential":"[credential redacted]"');
    }
    if (request && mimeCheckList.includes(request.postData?.mimeType)) {
      request.postData.text = request.postData.text.replace(/"password":"[a-zA-Z0-9?;!@#$%^&*()_+<>{}|]+"/, '"password":"[password redacted]"');
      request.postData.text = request.postData.text.replaceAll(/"url":"[a-zA-Z0-9?;\\/!@#$%^&*()_+-=<>{}|]+"/g, '"url":"[url redacted]"');
      request.postData.text = request.postData.text.replaceAll(/"auth":"[a-zA-Z0-9?;\\/!@#$%^&*()_+\-=,<>{}|.:?]+"/g, '"auth":"[auth redacted]"');
    }
  }
...
function removeURLContent(input, URLSchemaList) {
  ...
  for (const entry of entries) {
    for (const schema of URLSchemaList) {
      if (schema.redactRequest && entry.url.test(schema.regex))
        entry.request.postData.text = entry.request?.postData.text.replace(schema.regex, "[body redacted]");
      if (schema.redactResponse && entry.url.test(schema.regex))
        entry.response.content.text = entry.response?.content.text.replace(schema.regex, "[body redacted]");
    }
  }
  return JSON.stringify(harJSON, null, 2);
}
...
// ../.wrangler/tmp/pages-5J1lww/functionsRoutes-0.7628930951999036.mjs
var routes = [
  {
    routePath: "/scrub",
    mountPath: "/",
...
// ../.wrangler/tmp/bundle-aBXFsR/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-aBXFsR/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.611668495033761.mjs.map
